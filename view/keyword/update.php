...
@foreach ($keyword_categories as $id => $keyword_category)
                        <option value='{{ $id }}' {{ $id  === $keyword->keyword_category_id?'selected':'' }}>{{ $keyword_category->display_for_keywords_keyword_category() }}</option>
@endforeach
                    </select>
                  </div>
                </div>
              ...
              <div class="layui-row layui-col-space10 layui-form-item">
                <div class="layui-col-lg6">
                  ...
                  <div class="layui-input-block">
                    <input type="text" name="name" lay-verify="required" placeholder="" autocomplete="off" class="layui-input" value='{{ $keyword->name }}'>
                  </div>
                </div>
              </div>
